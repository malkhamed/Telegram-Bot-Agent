...
// Ensure agent_id is stored during purchases
function storeAgentIdInPurchase($user_id, $agent_id, $invoice_id) {
    global $pdo;
    $stmt = $pdo->prepare("UPDATE invoice SET agent_id = ? WHERE id_invoice = ? AND id_user = ?");
    $stmt->execute([$agent_id, $invoice_id, $user_id]);
}
